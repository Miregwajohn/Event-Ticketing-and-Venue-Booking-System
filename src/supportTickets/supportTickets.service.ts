...
// Scoped fetch for current user
export const getSupportTicketsByUserService = async (userId: number): Promise<TSupportTicketsSelect[]> => {
  return await db.query.supportTickets.findMany({
...
export const updateSupportTicketService = async (
  id: number,
  data: Partial<TSupportTicketsInsert>
): Promise<string> => {
  try {
